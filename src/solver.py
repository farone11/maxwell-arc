    ...
    def radiation_pattern(self, grid, source_pos, phi, color):
        ...
        for y in range(h):
            for x in range(w):
                ...
                if intensity > 0.5: # Threshold Malus
                    out[y, x] = color
        return out
    
    def solve(self, train_pairs, test_input):
        train_in = [np.array(p[0]) for p in train_pairs]
        train_out = [np.array(p[1]) for p in train_pairs]
        
        # 1. Infer polarisasi dari demo pertama
        phi = self.infer_polarization_angle(train_in[0])
        
        # 2. Infer konservasi warna
...
if __name__ == "__main__":
    ...
    solver = MaxwellARCv2()
    print(f"{solver.name} Ready. {solver.score}")
